...
import { 
    exportBackup, 
    restoreBackup,
    onBackupProgress,      // ← ADICIONAR
    onRestoreProgress,     // ← ADICIONAR
    removeBackupProgressListener,   // ← ADICIONAR
    removeRestoreProgressListener,  // ← ADICIONAR
...
export default function SettingsDialog({ open, onOpenChange }: SettingsDialogProps) {
    ...
    useEffect(() => {
    // Listener para progresso de BACKUP
    onBackupProgress((progress) => {
        // MOSTRAR modal automaticamente quando receber primeiro evento
        ...
        setProgressData({
            ...
            status: progress.current >= 100 ? 'success' : 'loading',
        });

        // FECHAR modal automaticamente quando completar
    ...
    // Listener para progresso de RESTORE
    ...
    const handleExportBackup = async () => {
        ...
        try {
            // 1. Chamar exportBackup SEM mostrar modal ainda
            const result = await exportBackup();
            
            // 2. Se cancelou, sair silenciosamente
            if (!result.success && result.error === 'Cancelado pelo usuario') {
                setOperation('idle');
                return;
            }

            // 3. AGORA SIM mostrar progresso (arquivo já foi escolhido)
            setProgressData({
                type: 'backup',
                progress: 0,
                phase: 'Inicializando',
            ...
            // 4. Aguardar o progresso completar (progresso vem via IPC)
            // O progresso será atualizado pelo useEffect que você criou
            
            // 5. Quando terminar, fechar modal
            // (aguardar um pouco para pegar os últimos eventos de progresso)
   ...
   // ═══════════════════════════════════════════════
    ...
    const handleDoRestore = async () => {
        ...
        try {
            // 1. Chamar restoreBackup SEM mostrar modal ainda
            const result = await restoreBackup();

            // 2. Se cancelou ou falhou, sair
            if (!result?.success) {
                setOperation('idle');
                return;
            }
            ...
            setProgressData({
                ...
                progress: 0,
                phase: 'Inicializando',
                message: 'Preparando restauração...',
                status: 'loading',
            });
            setShowProgress(true);

            // 4. Aguardar progresso completar
            await new Promise(r => setTimeout(r, 500));
            setShowProgress(false);

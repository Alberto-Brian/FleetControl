// ========================================
// FILE: src/components/expense/EditExpenseCategoryDialog.tsx (REDESENHADO + BUG CORRIGIDO)
// ========================================
...
import { updateExpenseCategory } from '@/helpers/expense-category-helpers';
import { IUpdateExpenseCategory } from '@/lib/types/expense-category';
...
export default function EditExpenseCategoryDialog({ open, onOpenChange }: EditExpenseCategoryDialogProps) {
  const { t } = useTranslation();
  const { showSuccess, handleError } = useErrorHandler();
  const { state: { selectedCategory }, updateCategory } = useExpenses();
  
  const [isLoading, setIsLoading] = useState(false);
  const [formData, setFormData] = useState<IUpdateExpenseCategory>({
    name: '',
    description: '',
    type: 'operational',
    color: '#3b82f6',
  ...
  // ✨ Sync com selectedCategory quando dialog abre
  // ⚠️ BUG CORRIGIDO: useEffect SEMPRE deve ser chamado (não pode estar após early return)
  useEffect(() => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const updated = await updateExpenseCategory(selectedCategory!.id, formData);
      
      updateCategory(updated);
      showSuccess('expenses:toast.categoryUpdateSuccess');
      onOpenChange(false);
    } catch (error: any) {
      handleError(error, 'expenses:toast.categoryUpdateError');
    } finally {
      setIsLoading(false);
    }
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription>
            {t('expenses:dialogs.editCategory.description')}
          </DialogDescription>
        </DialogHeader>

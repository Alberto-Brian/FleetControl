...
function GeofenceDetail({
  ...
  const { state, linkedImeis } = useTracking();
  ...
  const unlinkedCount = allDevices.filter(d => !linkedImeis.has(d.uniqueId)).length;

  const filteredDevices = useMemo(() => {
    let list = allDevices;
    if (!showUnlinked) {
      list = list.filter(d => linkedImeis.has(d.uniqueId));
    }
    ...
    // Atribuídos primeiro, depois ordenação alfabética
    return [...list].sort((a, b) => {
      const aA = assignedIds.has(a.id) ? 0 : 1;
      const bA = assignedIds.has(b.id) ? 0 : 1;
      return aA - bA || a.name.localeCompare(b.name, 'pt');
    });
  }, [allDevices, linkedImeis, showUnlinked, deviceSearch, assignedIds]);
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5 px-0.5">
              <p className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground/60">
                {t('geofences.devices', 'Dispositivos')}
              </p>
              {!loadingDevices && (
                <span className="text-[10px] text-muted-foreground/50">
                  {assignedDevices.length} {t('geofences.devicesActive', 'activo(s)')}
                </span>
              )}
            </div>
            ...
            {loadingDevices ? (
              ...
              <p className="text-xs text-muted-foreground/50 py-2 px-0.5">
                {t('geofences.noDevices', 'Sem dispositivos sincronizados')}
              </p>
            ) : (
              <>
                {/* Pesquisa + toggle */}
                <div className="space-y-1.5 mb-2">
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder={t('geofences.searchDevice', 'Pesquisar veículo...')}
                      className="w-full h-7 pl-7 pr-6 text-xs rounded-md border border-input bg-background focus:outline-none focus:ring-1 focus:ring-ring"
                    />
                    {deviceSearch && (
                      <button
                  ...
                  {unlinkedCount > 0 && (
                    <label className="flex items-center gap-1.5 cursor-pointer select-none">
                      ...
                      <span className="text-[10px] text-muted-foreground/60">
                        {t('geofences.showUnlinked', 'Incluir {{count}} sem veículo associado', { count: unlinkedCount })}
                      </span>
                    </label>
                  )}
                </div>
                ...
                {filteredDevices.length === 0 ? (
                  <p className="text-xs text-muted-foreground/50 py-2 text-center">
                    {t('geofences.noDevicesMatch', 'Sem resultados')}
                  </p>
                ) : (
                      ...
                      return (
                        ...
                        >
                          ...
                          <button
                            ...
                            disabled={isToggling}
                            title={isAssigned
                              ? t('geofences.removeDevice', 'Remover desta zona')
                              : t('geofences.addDevice',    'Adicionar a esta zona')}
                          >
                            {isToggling
...
export function GeofencePanel({ isOpen, onStartDraw, onFocusGeofence, onClose }: Props) {
  const { t }       = useTranslation('tracking');
  ...
  useEffect(() => {
    if (!isOpen) { setSelected(null); setEditing(null); setModalOpen(false); }
  }, [isOpen]);

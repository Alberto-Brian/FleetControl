...
export default function DriversPageContent() {
  ...
  // Dados para selects das viagens agendadas
  ...
  // Carregar veículos e rotas quando muda para aba de viagens
  useEffect(() => {
    if (activeTab === 'scheduled_trips' && vehiclesList.length === 0) {
  ...
  async function loadVehiclesAndRoutes() {
    try {
      ...
      // silencioso — veículos/rotas são opcionais para o form
    }
  }
        ...
        <div className="min-h-screen bg-slate-50/50 dark:bg-transparent -m-6 p-6">
          <div className="max-w-[1500px] mx-auto space-y-8 pb-10">
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-3">
              {/* TabsList alargada para 2 abas */}
              <TabsList className="grid w-full max-w-[520px] grid-cols-2 bg-muted/60 p-1 rounded-xl border border-muted/50">
                <TabsTrigger
                  value="drivers"
                ...
                {/* <TabsTrigger
                  value="scheduled_trips"
                  className="rounded-lg py-2.5 text-sm font-bold transition-all data-[state=active]:bg-background data-[state=active]:text-primary data-[state=active]:shadow-sm flex items-center gap-2"
                >
                  <Truck className="w-4 h-4" />
                  {t('drivers:tabs.scheduledTrips', 'Viagens')}
                </TabsTrigger> */}
              </TabsList>
                ...
                <div className="bg-card rounded-2xl border border-muted/50 shadow-sm overflow-hidden">
                  ...
                  {paginationInfo.totalPages > 1 && (
                    <div className="border-t border-muted/40 px-3 py-2 flex justify-end">
                      <Pagination pagination={paginationInfo} onPageChange={handlePageChange} onLimitChange={handleLimitChange} />
                    </div>
                  )}
                </div>

// ========================================
// FILE: src/i18n/locales/en/settings.en.ts
// ========================================

export const enSettings = {
  title: 'Settings',
  saved: 'Saved',
  saving: 'Saving...',
  save: 'Save',
  error: 'Error',
  unsavedChanges: 'You have unsaved changes. Click <strong>Save</strong> to apply.',
  resetToDefaults: 'Reset to defaults',
  resetConfirm: 'Are you sure you want to reset all settings to their default values?',
  resetSuccess: 'Settings reset successfully.',
  reloadApp: 'Reload application',

  nav: {
    appearance:     'Appearance',
    language:       'Language',
    company:        'Company',
    pdf:            'PDF Reports',
    alerts:         'Alerts',
    trips:          'Trips',
    fuel:           'Fuel',
    system:         'System',
    backups:        'Backups',
    databases:      'Databases',
    about:          'About',
    license:        'License',
    geofenceAlerts: 'GPS Alerts',
    server:         'Server',
    views:          'Views',
  },

  // ── Databases ─────────────────────────────────────────────────────────────
  databases: {
    title:           'Databases',
    subtitle:        'Browse and activate historical databases in read-only mode to generate reports.',
    noData:          'No databases found.',
    loading:         'Loading...',
    records:         'total records',
    current:         'Active',
    activate:        'Activate for reports',
    deactivate:      'Deactivate',
    activated:       'In use (historical)',
    readingStats:    'Reading stats...',
    statsError:      'Error reading stats.',
    tables: {
      vehicles:      'Vehicles',
      drivers:       'Drivers',
      trips:         'Trips',
      refuelings:    'Refueling',
      maintenances:  'Maintenance',
      expenses:      'Expenses',
      fines:         'Fines',
      users:         'Users',
      routes:        'Routes',
    },
    delete:                    'Delete',
    deleteConfirmTitle:        'Confirm Deletion',
    deleteConfirmText:         'This action is irreversible. Enter your password to confirm.',
    deletePasswordLabel:       'Password',
    deletePasswordPlaceholder: 'Enter your password',
    deleteSuccess:             'Database deleted successfully.',
    deleteErrorNotFound:       'File not found.',
    deleteErrorActive:         'Cannot delete the active database.',
    deleteErrorPassword:       'Incorrect password.',
    deleteErrorGeneric:        'Error deleting database.',
    deleteConfirmBtn:          'Delete',
    deleteCancelBtn:           'Cancel',
    backupsSection:            'Automatic Backups',
    backupsSubtitle:           'Databases stored in automatic backup copies. Activate one for read-only historical access.',
    backupsLoading:            'Loading backups...',
    backupsNoData:             'No backup copies found.',
    backupFolderLabel:         'Backup from',
    backupDatabaseCount:       'database(s)',
    activateBackupDb:          'Activate for reading',

    // ── Database size alert (visible to all)
    dbSizeAlertTitle: 'Large database',
    dbSizeAlertText:  'The active database is {{size}} MB. Consider managing its size in Settings › Databases (developer zone).',

    // ── Developer Zone
    devZoneTitle:        'Developer Zone',
    devZoneLockHint:     'This area is reserved for the system developer. Enter the PIN to access database management tools.',
    devZonePinPlaceholder: 'PIN',
    devZoneUnlockBtn:    'Unlock',
    devZoneWrongPin:     'Incorrect PIN. Please try again.',
    devZoneLockBtn:      'Lock area',

    // ── Automatic Database Management
    dbMgmtTitle:      'Automatic Database Management',
    dbMgmtDesc:       'Configure how the system manages local database growth over time.',
    dbMgmtSaveBtn:    'Save Configuration',
    dbMgmtSaveSuccess:'Configuration saved. It will take effect on the next application start.',
    dbMgmtSaveError:  'Error saving configuration.',

    // Modes
    mode_disabled:      'Disabled',
    mode_disabled_desc: 'No automatic management. Manage space manually by deleting old records when needed. The system only alerts when the database is getting large.',
    mode_rotation:      'Rotation',
    mode_rotation_desc: 'Automatically creates a new database when the configured limit is reached, archiving the current one and copying recent data.',
    mode_retention:     'Data Retention',
    mode_retention_desc:'Periodically deletes old records from transactional tables, keeping only data within the configured retention period.',

    // Rotation
    rotationSettings:        'Rotation Parameters',
    rotMaxSizeLabel:         'Maximum size',
    rotMaxSizeDesc:          'Creates a new database when the file exceeds this size.',
    rotMaxAgeLabel:          'Maximum age',
    rotMaxAgeDesc:           'Creates a new database when the current one is older than this number of days.',
    rotTransitionDaysLabel:  'Data to copy on rotation',
    rotTransitionDaysDesc:   'How many days of transactional data are copied to the new database.',
    rotForceBtn:             'Force Rotation Now',
    rotForceConfirmTitle:    'Confirm Forced Rotation',
    rotForceConfirmText:     'A new active database will be created and the current one archived. The most recent data will be copied according to the configuration. This action is irreversible.',
    rotForceConfirmBtn:      'Yes, rotate now',
    rotForceSuccess:         'Rotation completed successfully.',
    rotForceError:           'Error rotating database',

    // Retention
    retentionSettings:           'Tables and Retention Period',
    retentionDesc:               'Select which tables to manage and how many days of data to keep in each.',
    retentionApplyBtn:           'Apply Retention Now',
    retentionApplyConfirmTitle:  'Confirm Retention Application',
    retentionApplyConfirmText:   'Records older than the configured period will be permanently deleted from the selected tables. This action is irreversible.',
    retentionApplyConfirmBtn:    'Yes, delete old data',
    retentionApplySuccess:       'Retention applied successfully.',
    retentionApplyError:         'Error applying retention',
    retentionDaysLabel:          'days',

    // Rotation alerts
    rotationNeededTitle:      'Automatic Rotation Required',
    rotationNeededText:       'The database has reached the configured limits. Rotation will occur automatically on the next application start.',
    rotationApproachingTitle: 'Database Approaching Limit',
    rotationApproachingText:  'The active database is {{size}} MB of the {{max}} MB configured (80%+). Automatic rotation will occur soon.',

    // Table classification
    tableClassificationTitle:   'Table Classification',
    masterTablesLabel:          'Master — full copy on rotation',
    transactionalTablesLabel:   'Transactional — recent data copy on rotation',

    // Transactional table names
    table_trips:             'Trips',
    table_maintenances:      'Maintenance',
    table_maintenance_items: 'Maintenance Items',
    table_expenses:          'Expenses',
    table_refuelings:        'Refueling',
    table_fines:             'Fines',

    // Master table names
    table_vehicles:               'Vehicles',
    table_drivers:                'Drivers',
    table_users:                  'Users',
    table_routes:                 'Routes',
    table_fuel_stations:          'Fuel Stations',
    table_vehicle_categories:     'Vehicle Categories',
    table_maintenance_categories: 'Maintenance Categories',
    table_expense_categories:     'Expense Categories',
    table_categories:             'Categories',
    table_workshops:              'Workshops',
    table_company_settings:       'Company Settings',
    table_vehicle_documents:      'Vehicle Documents',
  },

  // ── License ───────────────────────────────────────────────────────────────
  license: {
    title:          'License',
    subtitle:       'License information and management',
    active:         'Active License',
    reactivate:     'Reactivate',
    modeStandalone: 'Standalone (Offline)',
    modeConnected:  'Connected (Online)',
    typeTrial:      'Trial',
    typeBasic:      'Basic',
    typeProfessional:'Professional',
    typeEnterprise: 'Enterprise',
    client:         'Client',
    email:          'Email',
    nif:            'Tax ID',
    users:          'Users',
    usersUpTo:      'Up to {{max}}',
    validity:       'Validity',
    validityDays:   '({{days}} days)',
    features:       'Features',
    dangerZone:     'Danger Zone',
    removeWarning:  'Removing the license blocks system access. You can only re-enter by activating a new license.',
    remove:         'Remove License',
    removeConfirm:  'Are you sure you want to remove the license? The system will be locked until you activate a new one.',
    noLicense:      'No active license',
    noLicenseFound: 'No license found.',
    activate:       'Activate License',
  },

  // ── Server ────────────────────────────────────────────────────────────────
  server: {
    title:          'Server Configuration',
    subtitle:       'FleetControl API server URL',
    apiUrl:         'API URL',
    apiUrlDesc:     'FleetControl server base address (e.g. http://192.168.1.10:3001)',
    apiUrlPlaceholder: 'http://localhost:3001',
    saved:          'URL saved. Restart the app to apply.',
    testConnection: 'Test connection',
    testing:        'Testing...',
    connectionOk:   'Connection successful',
    connectionFail: 'Cannot reach server',
  },

  appearance: {
    title:            'Theme',
    description:      'Choose between light or dark theme',
    label:            'Appearance',
    subtitle:         'Light or dark mode',
    customization:    'Customization',
    customSubtitle:   'Adjust the interface to your preferences',
    animations:       'Animations',
    transparency:     'Transparency effects',
    enabled:          'Enabled',

    // Font family
    typography:         'Font family',
    typographyDesc:     'Application typeface',
    fontPreview:        'Fleet 2025 · 123',
    font_geist_label:   'Geist',
    font_geist_desc:    'Modern and clean',
    font_system_label:  'System',
    font_system_desc:   'Native OS font',
    font_serif_label:   'Serif',
    font_serif_desc:    'Classic and elegant',
    font_mono_label:    'Monospace',
    font_mono_desc:     'Ideal for technical data',

    // Font size
    fontSize:             'Font size',
    fontSizeDesc:         'Affects menus, submenus and all content',
    fontsize_sm:          'Small',
    fontsize_sm_desc:     'Compact',
    fontsize_md:          'Normal',
    fontsize_md_desc:     'Default',
    fontsize_lg:          'Large',
    fontsize_lg_desc:     'Comfortable',
    fontsize_xl:          'Extra large',
    fontsize_xl_desc:     'Accessible',

    // Glass panel (connected mode)
    glassPanel:           'Background panel',
    glassPanelDesc:       'Transparency and blur of the floating panel (connected mode)',
    glassOpacity:         'Opacity',
    glassBlur:            'Blur',
    glassTransparent:     'Transparent',
    glassSolid:           'Solid',
    glassNoBlur:          'No blur',
    glassMaxBlur:         'Maximum',
    glassReset:                  'Reset to defaults',
    glassResetDark:              'Dark mode',
    glassResetLight:             'Light mode',
    glassSaveAsDefault:          'Save as default',
    glassSaveDark:               'Save (dark)',
    glassSaveLight:              'Save (light)',
    glassResetOnThemeChange:     'Reset when switching theme',
    glassResetOnThemeChangeDesc: 'Automatically applies the new theme\'s defaults when switching between light and dark',

    // Sidebar menu
    sidebarTitle:         'Side menu',
    sidebarDesc:          'Configure the navigation menu behaviour',
    sidebarCollapsed:          'Start collapsed',
    sidebarCollapsedDesc:      'Show only icons when opening the application',
    sidebarAutoCollapse:       'Auto-collapse on hover',
    sidebarAutoCollapseDesc:   'Expands on mouse-over and collapses when the cursor leaves — the collapse button disappears',

    // Layout
    layoutTitle:          'Interface layout',
    layoutDesc:           'Controls the padding and rounded corners of the interface',
    layoutPadding:        'Padding & rounded corners',
    layoutPaddingDesc:    'Keep padding and rounded corners on the content panel',

    // Map
    mapTitle:             'Tracking map',
    mapDesc:              'Customize how devices appear on the map',
    mapLabelType:         'Vehicle label on map',
    mapLabelPlate:        'Plate',
    mapLabelBrandModel:   'Brand/Model',
    mapLabelBoth:         'Both',
    mapAnimateMarkers:    'Animate markers',
    mapAnimateMarkersDesc:'Animate icons when adding them to the map',
    mapPulseMarkers:      'Marker pulse',
    mapPulseMarkersDesc:  'Slow heartbeat animation on vehicle icons on the map',

    // Filters
    filtersTitle:         'List preferences',
    filtersDesc:          'Controls the behaviour of filters and view mode across listing pages',
    persistFilters:       'Save filters between sessions',
    persistFiltersDesc:   'Keeps active filters when closing and reopening the app',
    persistViewMode:          'Save view mode',
    persistViewModeDesc:      'Keeps the cards/list choice when closing and reopening the app',
    persistPageSize:          'Save items per page',
    persistPageSizeDesc:      'Remembers how many items to show per page in each listing',
    persistCurrentPage:       'Save current page',
    persistCurrentPageDesc:   'Resumes the last visited page when reopening a listing',
    showFilterCounts:         'Show counts in filters',
    showFilterCountsDesc:     'Displays the total number of records for each option in filter menus (e.g. "Passenger Light (55)"). Values are global totals and do not change when other filters are active.',
  },

  language: {
    title:          'Application language',
    description:    'Select your preferred language for the interface',
    label:          'Language',
    subtitle:       'Português / English',
    region:         'Region & Format',
    regionSubtitle: 'Localization settings',
    dateFormat:     'Date format',
    dateValue:      'DD/MM/YYYY',
    timeFormat:     'Time format',
    timeValue:      '24 hours',
  },

  company: {
    title:          'Company Details',
    description:    'Information that appears in PDF reports and generated documents',
    logo:           'Company logo',
    logoDescription:'Appears in the header of all PDF reports.',
    logoHint:       'PNG, JPG, WEBP or SVG · Max 2 MB',
    logoClick:      'Click or drag',
    logoChange:     'Change',
    logoUpload:     'Upload',
    logoRemove:     'Remove',
    identification: 'Identification',
    name:           'Company Name',
    namePlaceholder:'Ex: ABC Company, Ltd',
    taxId:          'Tax ID',
    taxIdPlaceholder:'Ex: 5401234567',
    phone:          'Phone',
    phonePlaceholder:'+244 923 000 000',
    email:          'Email',
    emailPlaceholder:'info@company.com',
    address:        'Address',
    street:         'Street',
    streetPlaceholder:'Street / Avenue, No.',
    city:           'City',
    cityPlaceholder:'Luanda',
    state:          'Province / State',
    statePlaceholder:'Luanda',
    postalCode:     'Postal Code',
    postalPlaceholder:'0000-000',
    regionalization:'Regionalization',
    currency:       'Currency',
    timezone:       'Timezone',
    errors: {
      nameRequired: 'Company name is required.',
      logoSize:     'Logo cannot exceed 2 MB.',
      logoFormat:   'Unsupported format. Use PNG, JPG, WEBP or SVG.',
    },
  },

  views: {
    title:                  'Views',
    description:            'Configure what is shown in each section of the app.',

    analyticsPanels:        'Analytics Panels',
    analyticsPanelsDesc:    'Show charts and metrics in each view. Position is configurable below.',

    vehicles:               'Vehicles',
    vehiclesDesc:           'Shows fleet utilisation %, availability %, average mileage and category breakdown.',
    drivers:                'Drivers',
    driversDesc:            'Shows driver availability %, on-trip count, on-leave count and licences expiring in 30 days.',
    trips:                  'Trips',
    tripsDesc:              'Shows trip completion %, total distance driven and status breakdown.',
    fuel:                   'Fuel',
    fuelDesc:               'Shows total cost, litres fuelled and average price per litre (no percentages).',
    maintenance:            'Maintenance',
    maintenanceDesc:        'Shows total cost, scheduled and in-progress interventions (no percentages).',
    expenses:               'Expenses',
    expensesDesc:           'Shows payment rate %, total amount and category breakdown.',

    metricsTitle:            'Metric Definitions',
    metricsDesc:             'What each indicator and percentage in the analytics panels represents.',

    metricsVehiclesM1:       '% Utilisation',
    metricsVehiclesM1Desc:   'Active vehicles ÷ fleet total. Reflects the fleet\'s immediate operational load.',
    metricsVehiclesM2:       '% Availability',
    metricsVehiclesM2Desc:   'Ready, unassigned vehicles ÷ total. Indicates spare fleet capacity.',
    metricsVehiclesM3:       'Avg mileage',
    metricsVehiclesM3Desc:   'Average km recorded per vehicle — helps spot uneven usage across the fleet.',

    metricsDriversM1:        '% Availability',
    metricsDriversM1Desc:    'Drivers with no active trip ÷ total. Indicates how many are ready for new assignment.',
    metricsDriversM2:        'On trip',
    metricsDriversM2Desc:    'Number of drivers currently on an active trip.',
    metricsDriversM3:        'On leave',
    metricsDriversM3Desc:    'Drivers on active annual leave — temporarily unavailable for assignment.',
    metricsDriversM4:        'Expiring licences',
    metricsDriversM4Desc:    'Drivers whose driving licence expires in the next 30 days — requires urgent renewal.',

    metricsTripsM1:          '% Completion Rate',
    metricsTripsM1Desc:      'Successfully completed trips ÷ total (including cancelled and in-progress). High value indicates good operational execution.',
    metricsTripsM2:          'Total distance',
    metricsTripsM2Desc:      'Total km driven by all vehicles in the selected period.',

    metricsFuelM1:           'Total cost',
    metricsFuelM1Desc:       'Sum of all refuelling costs in the period. Excludes other operating expenses.',
    metricsFuelM2:           'Litres fuelled',
    metricsFuelM2Desc:       'Total volume of fuel recorded. Dividing by the total cost gives the average price.',
    metricsFuelM3:           'Avg price/litre',
    metricsFuelM3Desc:       'Average cost per litre in the period — useful for tracking fuel price trends.',

    metricsMainM1:           'Total cost',
    metricsMainM1Desc:       'Sum of all maintenance costs (parts + labour) recorded in the period.',
    metricsMainM2:           'Scheduled',
    metricsMainM2Desc:       'Planned maintenances not yet started. A high count may indicate a growing backlog.',
    metricsMainM3:           'In progress',
    metricsMainM3Desc:       'Active interventions right now — these vehicles may be temporarily unavailable.',

    metricsExpM1:            '% Payment Rate',
    metricsExpM1Desc:        'Settled expenses ÷ total registered. High value indicates good payment management.',
    metricsExpM2:            'Total amount',
    metricsExpM2Desc:        'Sum of all expenses (paid and pending) in the selected period, across all categories.',

    layoutTitle:            'Panel Layout',
    layoutDesc:             'Positioning of analytics panels relative to the list.',
    layoutPosition:         'Position',
    layoutPositionDesc:     'Vertical places the panel to the right of the list; horizontal places it above.',
    layoutVertical:         'Vertical',
    layoutHorizontal:       'Horizontal',
  },

  pdf: {
    title:              'PDF Reports',
    description:        'Customize the appearance and content of generated reports',

    watermark:          'Watermark',
    watermarkDesc:      'Add diagonal text to all PDF reports',
    watermarkEnabled:   'Enable watermark',
    watermarkUseLogo:   'Use logo as watermark',
    watermarkUseLogoDesc: 'Uses the configured logo instead of text. If no logo is set, text will be used.',
    watermarkText:      'Watermark text',
    watermarkTextPlaceholder: 'Ex: CONFIDENTIAL, DRAFT',
    watermarkOpacity:   'Opacity',
    watermarkOpacityHint:'0.05 (subtle) → 0.30 (visible)',

    colors:             'Report Colors',
    colorsDesc:         'Define colors used in PDF headers, titles and badges',
    primaryColor:       'Primary Color',
    primaryColorHint:   'Headers, titles, status badges',
    secondaryColor:     'Secondary Color',
    secondaryColorHint: 'Helper texts, labels',
    colorPreview:       'Preview',

    layout:             'Layout & Content',
    layoutDesc:         'Configure what is displayed in the reports',
    showCharts:         'Include charts',
    showChartsDesc:     'Adds visual charts (bar, pie, line) to PDF reports',
    showFooter:         'Show footer with pagination',
    showSummary:        'Show executive summary',
    paperSize:          'Paper size',
    orientation:        'Orientation',
    portrait:           'Portrait',
    landscape:          'Landscape',

    valueFormat: 'Value Format',
    valueFormatDesc: 'Choose how large numbers are displayed in reports',
    valueFormatCompact: 'Compact (K/M)',
    valueFormatFull: 'Full numbers',

    showCurrency: 'Show Currency Symbol',
    showCurrencyDesc: 'Display the Kz symbol next to monetary values'
  },

  alerts: {
    title:        'Alert Settings',
    description:  'Define when and how the system should alert about important events',

    mileage:         'Mileage Review',
    mileageDesc:     'Alert when a vehicle reaches the km limit without a review',
    mileageThreshold:'Km alert threshold',
    mileageUnit:     'km',

    license:         'Driving License Validity',
    licenseDesc:     'Alert in advance when a license is about to expire',
    licenseDays:     'Days in advance',

    insurance:       'Insurance Validity',
    insuranceDesc:   'Alert when a vehicle\'s insurance is about to expire',
    insuranceDays:   'Days in advance',

    maintenance:     'Preventive Maintenance',
    maintenanceDesc: 'Enable alerts for scheduled preventive maintenance',
    maintenanceEnabled: 'Maintenance alerts',

    fines:           'Overdue Fines',
    finesDesc:       'Enable alerts for fines with overdue payment',
    finesEnabled:    'Fine alerts',

    daysUnit:        'days',
  },

  trips: {
    title:       'Trip Settings',
    description: 'Define rules and restrictions for trip management',

    approval:       'Trip Approval',
    approvalDesc:   'Require supervisor approval before starting a trip',
    approvalEnabled:'Require approval',

    signature:      'Digital Signature',
    signatureDesc:  'Require driver\'s digital signature at the end of each trip',
    signatureEnabled:'Require signature',

    speed:          'Speed Limit',
    speedDesc:      'Log an alert when the maximum speed is exceeded (0 = no limit)',
    speedLabel:     'Maximum speed (km/h)',
    speedUnit:      'km/h',
    speedHint:      'Set 0 to disable speed control',
  },

  fuel: {
    title:       'Fuel Settings',
    description: 'Fuel level control and alerts',

    levelControl:    'Level Control',
    levelControlDesc:'Enable fuel level monitoring for vehicles',
    levelEnabled:    'Enable level control',

    minLevel:        'Minimum Alert Level',
    minLevelDesc:    'Alert when fuel level falls below this value',
    minLevelLabel:   'Minimum level (%)',
    minLevelUnit:    '%',
  },

  system: {
    title:       'System Settings',
    description: 'Sessions, security and audit',

    session:         'Session & Security',
    sessionDesc:     'Configure user session behavior',
    timeout:         'Inactivity timeout',
    timeoutLabel:    'Minutes (0 = no limit)',
    timeoutUnit:     'min',
    timeoutHint:     'Set 0 to keep the session always active',
    multiLogin:      'Multiple sessions',
    multiLoginDesc:  'Allow the same user to be logged in on multiple devices',

    notifications:   'Notifications',
    notificationsDesc:'Enable system notifications in the taskbar',
    notificationsEnabled: 'Enable notifications',

    audit:           'Audit',
    auditDesc:       'Detailed log of all user actions',
    auditEnabled:    'Enable audit log',
    auditRetention:  'Log retention',
    auditRetentionLabel: 'Retention days',
    auditRetentionUnit:  'days',

    reset:           'Reset Settings',
    resetDesc:       'Resets all system settings to factory defaults. This action cannot be undone.',
    resetButton:     'Reset to defaults',
    resetWarning:    'Warning: this action cannot be undone.',
  },

  backups: {
    title:           'Backup Management',
    description:     'Configure and manage system backups',
    auto:            'Automatic Backup',
    autoDesc:        'Daily automatic database backups. The last 7 backups are kept automatically.',
    frequency:        'Frequency',
    frequencyValue:   'Daily',
    frequencyWeekly:  'Weekly',
    kept:             'Backups kept',
    keptValue:        'Last 7',
    keptUnit:         'copies',
    lastBackup:       'Last backup',
    lastBackupValue:  'Today, 03:00',
    noBackupYet:      'No backups yet',
    listTitle:        'Automatic Backups',
    listCount:        'copies found',
    noBackupsFound:   'No automatic backups found. Backups are created automatically when the app starts.',
    latestBadge:      'latest',
    manual:          'Manual Backup',
    manualDesc:      'Export or restore complete system backups',
    export:          'Export Backup',
    exportDesc:      'Creates a ZIP file with all data for transfer to another computer',
    restore:         'Restore Backup',
    restoreDesc:     'Imports a backup from another computer. Requires license reactivation.',
    warning:             '<strong>Important:</strong> Restoring a backup will <strong>permanently replace all current data</strong> with the backup data — this action is irreversible. Restore is restricted to the system developer. If restoring on another computer, license reactivation with that computer\'s Machine ID will be required.',
    restoreDevOnly:      'Restricted access',
    restoreDevOnlyHint:  'Restore is restricted to the developer. Unlock the developer zone in Settings › Databases to restore.',
    advanced:        'Advanced Tools (Testing)',
    advancedDesc:    'These actions are for testing and maintenance only. Do not use in production without technical guidance.',
    rotation:        'Force database rotation',
    rotationDesc:    'Creates a new active database and archives the current one. Useful only for rotation and backup tests.',
    rotationButton:  'Force rotation now',
    restoreFromBackup:         'Restore',
    confirmAutoRestore:        'Confirm backup restore',
    confirmAutoRestoreText:    'You are about to restore the backup from:',
    confirmAutoRestoreBullet1: 'Current data will be replaced by this backup',
    confirmAutoRestoreBullet2: 'A safety backup of current data will be created first',
    confirmAutoRestoreBullet3: 'The application will reload automatically',
    confirmRestore:  'Confirm restore',
    confirmRestoreText: 'Are you sure you want to restore a backup?',
    confirmBullet1:  'Current data will be replaced',
    confirmBullet2:  'License reactivation will be required',
    confirmBullet3:  'The application will restart automatically',
    confirmIrreversible: 'This action cannot be undone.',
    confirmCancel:   'Cancel',
    confirmYes:      'Yes, restore',
    progressBackup:  'Creating backup...',
    progressRestore: 'Restoring backup...',
    progressInit:    'Initializing...',
    progressPrepare: 'Preparing files...',
    progressDb:      'Databases',
    progressDbMsg:   'Copying databases...',
    progressConfig:  'Configuration',
    progressConfigMsg:'Copying user files...',
    progressZip:     'Compression',
    progressZipMsg:  'Creating ZIP archive...',
    progressDone:    'Done',
    progressSuccess: 'Backup created successfully',
    progressValidate:'Validation',
    progressValidateMsg:'Checking integrity...',
    progressExtract: 'Extraction',
    progressExtractMsg:'Extracting contents...',
    progressRestoreDb:'Restoration',
    progressRestoreDbMsg:'Restoring databases and settings...',
    progressFinalize:'Finalization',
    progressFinalizeMsg:'Applying changes...',
    progressRestoreDone:'Restore completed',
    dontClose:       'Do not close this window',
    exportSuccess:   '✅ Backup exported',
    exportSuccessMsg:'Backup created successfully! ({{size}} MB)',
    exportError:     '❌ Export error',
    restoreSuccess:  'Backup restored',
    restoreSuccessMsg:'Restore completed. The application will restart shortly...',
    restoreSuccessMsgNoRestart:'Restore completed successfully.',
    restoreError:    'Error during restore',
    importantWarning:'Important',
  },

  about: {
    appName:        'FleetControl',
    appSubtitle:    'Fleet Management System',
    version:        'Version',
    build:          'Build',
    buildValue:     '2026.07.16',
    lastUpdate:     'Last update',
    lastUpdateValue:'July 16, 2026',
    license:        'License',
    licenseValue:   'Proprietary',
    systemInfo:     'System Information',
    developedBy:    'Developed by',
    companyName:    'AKM Systems',
    companyDesc:    'Angolan company specialized in business management software development, offering modern technological solutions adapted to the national market.',
    location:       'Location',
    locationValue:  'Luanda, Angola',
    founded:        'Founded',
    foundedValue:   '2025',
    nif:            'Tax ID',
    nifValue:       '5401234567',
    commercial:     'Commercial Reg.',
    commercialValue:'RC-123456',
    contacts:       'Contacts',
    addressLabel:   'Address',
    addressLine1:   'Luanda, Angola',
    addressLine2:   '',
    copyright:      '© 2025–2026 AKM Systems. All rights reserved.',
    madeIn:         'Developed in Angola for Angola',
    loading:        'Loading...',
  },
} as const;

export default enSettings;